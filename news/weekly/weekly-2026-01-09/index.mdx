...
draft: false
excludeSearch: false
title: AUTO 周报 2026-01-09 - 2026-01-16
---

## 本周更新摘要

- 电机学（EE3001）：新增 24 秋与 25 秋相关资料，并补充课程结构化信息（EE3001.toml）。

- 电力系统分析（EE3015）/ 电力电子技术（EE3002）/ DSP 的原理与应用（EE3005）等电气类课程：陆续上传 23/24/25 年度教材与考试资料（含期末试题），并添加/更新对应的 .toml 结构化元数据。

- 计算机系统（COMP3052）：上新课程页面，补充课程教材与网课推荐，并上传 2025 春期末试卷；同时添加结构化课程信息（COMP3052.toml）。

- 设计与制造 B（MECH2020）：新建仓库并首次提交（课程上线）。

- 数值分析（MATH4004）：上传 25 秋学期数值分析期末试题。

- 机器人学基础（MECH3060）：添加 2025 春/秋学期作业、答案及考试试卷（含 2025 秋作业答案）。

- 嵌入式系统（AUTO3024）：新增课程评价与笔记。

- 跨专业选修课程体系（CrossSpecialty）：新增量子力学与波谱学原课程内容、作业与期末试卷；并添加空间环境、会计学原理等课程的分数组成信息。

- 离散数学（COMP2030）：补充一份定理与定义整理并合并他人提交的补丁/文件上传。

- 普通天文学（SPST1004）：上传 25 秋期末回忆试题。

- 马克思主义基本原理（GEIP1011）：补充教师信息与评价，并上传 2025 秋马原思维导图。

- 经济学原理（ECON2005F）：更新课程信息说明。

- 大量课程统一添加/更新结构化课程信息（.toml）：本周 CI/维护合并了大量自动生成或手工校正的课程元数据，覆盖众多计算机、自动化、电气、机械、数学与通识类课程（包括但不限于 操作系统、编译原理、数据库系统、计算机体系结构、机器学习、人工智能、软件工程等核心课程），以便统一检索与展示。

- 其它重要合并与维护：
  - 多个由 GitHub Actions 发起的自动化 PR 已合并，repos-management 更新了仓库列表。
  - 部分课程（如 COMP3053 汇编）对 LLM 自动生成内容进行了人工审核与清理。

- 其他琐碎维护：若干仓库进行了旧 toml 删除、格式调整、日期修正、README/小文件变更等基础性维护操作。

（注：以上为本周期内有实质教材/试卷/作业/结构化元数据新增或重要文档修改的主要更新。）
...
- Jiao Ziang 在 [汇编语言与接口技术](https://github.com/HITSZ-OpenAuto/COMP3053) 中提交了信息：Merge pull request #7 from HITSZ-OpenAuto/ci/auto-generate-toml-1768254919

- Jiao Ziang 在 [（示例）大学物理实验](https://github.com/HITSZ-OpenAuto/COMP3019) 中提交了信息：Merge pull request #7 from HITSZ-OpenAuto/ci/auto-generate-toml-1768255078

- Jiao Ziang 在 [（示例）大学物理实验](https://github.com/HITSZ-OpenAuto/COMP3039) 中提交了信息：添加名字与课程代码
...
- Jiao Ziang 在 [电路与电子学 I](https://github.com/HITSZ-OpenAuto/EE1018) 中提交了信息：docs: add structured course info (EE1018.toml)
...
- Jiao Ziang 在 [（示例）大学物理实验](https://github.com/HITSZ-OpenAuto/COMP3039) 中提交了信息：docs: add structured course info (COMP3039.toml)
...
- Jiao Ziang 在 [（示例）大学物理实验](https://github.com/HITSZ-OpenAuto/COMP3019) 中提交了信息：docs: add structured course info (COMP3019.toml)
...
- Jiao Ziang 在 [汇编语言与接口技术](https://github.com/HITSZ-OpenAuto/COMP3053) 中提交了信息：docs: add structured course info (COMP3053.toml)
...
- Jiao Ziang 在 [计算机系统](https://github.com/HITSZ-OpenAuto/COMP3052) 中提交了信息：docs: add structured course info (COMP3052.toml)
...
- Jiao Ziang 在 [大学物理](https://github.com/HITSZ-OpenAuto/PHYS1001) 中提交了信息：删除旧 toml

- Jiao Ziang 在 [大学物理实验](https://github.com/HITSZ-OpenAuto/PHYS1002) 中提交了信息：删除旧 toml

- Jiao Ziang 在 [电路实验 IA](https://github.com/HITSZ-OpenAuto/EE1012A) 中提交了信息：删除旧 toml
...
- Jiao Ziang 在 [大学物理实验](https://github.com/HITSZ-OpenAuto/PHYS1002) 中提交了信息：docs: add structured course info (PHYS1002.toml)
